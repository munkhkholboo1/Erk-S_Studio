...
- Ask the citizens, on their own phones. A project can now hold as many consultations as it needs, the way it holds albums. Each is created from a named template - the first is the partial master plan questionnaire, copied from the owner's own document rather than invented - and a consultation carries its ticked questions, its written-answer section last, and its own results. Studio prints a QR code and a link; whoever scans it fills the form in an ordinary mobile browser with nothing installed, and presses send. «Хариулт татах» brings the answers down into the project, and the charts and the breakdown redraw from them. Answers already held are recognised and not counted twice, and a test run can be set aside with one button - moved, never deleted, so the trial's answers stop reaching the public result without being destroyed.
- A consultation cannot be published shut. The template creates one closed, and a closed consultation makes the server refuse every submission - so a QR code could have been printed, handed out, and every answer turned away. There is now a button that opens it, the state is on the screen in words, and the code cannot be re-minted once a QR has been printed.
- Nothing a citizen writes is paraphrased. Their own words are stored exactly as typed. A number the phone could not read is no longer dropped in silence: the form refuses to send and names the question. Two people filling on one device are two respondents, because the form asks rather than assuming.
- The consultation page no longer closes Studio. Opening it crashed the application through a recursion that rebuilt itself on every refresh.
- The album is redrawn by CHANGE, not by a timer and not by opening a project. Opening a project could freeze the machine for ten minutes and grow the folder to 9.7 GB, drawing an album that was ALREADY correct, over and over. What is skipped and what is drawn is now recorded, so "it did not redraw" is a measurement rather than a stopwatch reading on a fast machine.
- Renders saved over the top reach the album. Saving a drawing under a name that already existed left the album showing the old one, and the failure covered its own tracks. Orphaned copies left behind by a rename are now found by reference and removed, and what was deleted - how many files, how many megabytes - is written on the album's own line and survives a reload.
- One raster rule, everywhere. 300 dpi is the album's rule, not one writer's setting: the portfolio and the board cards draw the very same files and neither was held to it, while a 16 000 pixel render went into the album at its own size. All three now come down to the album's density at q90, and a render that could not be brought down says so instead of passing silently.
- The masthead names whoever the work goes out as. For hours it read "Erk-S Studio / CLOUD ERA" in every state while the account button showed somebody else's name, so a person could work all afternoon believing they were themselves. Who you are acting as is now chosen from a list at the top of the account menu - your own name, and this machine's bot seat marked as one - and the device's state follows from that choice rather than being asked about separately.
- An owner on their own seated machine is the owner again. Their projects were not listed at all, the company menu had disappeared in both states, and every right was refused, because three different pieces of code asked what the MACHINE was when the question was who is acting. A bot's underlying owner is now shown by name wherever the bot appears, and an address is never printed where a name belongs.
- The bot's PIN is asked when the bot acts, not at every launch - the owner's decision, replacing a rule that was deliberate and is now written down as changed.
- A refusal the server named reaches the person. One named refusal came back six times in a single day and was never once seen: an empty table and a silent button. The diagnostic record that should have caught it was erasing itself whenever it could not read its own file. An empty list now says which of its reasons it is, and waiting is no longer silent - preparation names itself before it starts, reports its own time separately from drawing, and an expansion that cannot reach the network is visible.
- Work that a scan loses is named. The same loss showed or hid depending on how the scan was started, and two reporters each named a different pair of causes. Dropped, orphaned and refused are three counts with three separate words, and a library emptied by a cleanup is refilled instead of staying empty - the owner's two complaints were one event.
- A3 is the only concept cover. The earlier format and the A4 variant are gone, the cover takes its geometry from its own drawing, the header row is 8 mm, the approver's position is drawn, and 75 objects on the page are each accounted for by measurement rather than by eye.
- Officials are proposed from the project's address. A local directory, kept beside the company information so it is entered once and used by every project, fills the approval and review tables from the district an address names. A proposal only ever fills an empty slot: a line a person wrote is never overwritten, and the three different ways of finding nothing each say which one happened.
- 0.001.64 was built on 11 September and never reached the server, so nothing was given to anyone. Its entries are below and are part of this release.

## [0.001.64] - 2026-09-11

- Leave bot state on a machine the server is still holding. A computer whose seat record had gone from disk while the server went on holding it in bot state had no way out at all: every reading of "are we seated" answered no, so the menu showed the entries for a free machine and the way out was not among them. Asked to leave, you were told to manage the seat; asked to manage it, you were told to leave first. The exit is now offered on the wider fact - whether this machine has EVER been a seat, which its registered device key records and which outlives any seat record - and if that cannot be read at all the exit is shown anyway, because hiding it is what built the trap. Pressing it used to do nothing whatsoever: with no seat record there was no seat name, and the action returned in silence. It now asks the server for the name this machine cannot find for itself, and releases it by that. Both exits say exactly what goes and what stays, and what stays is your project files, your sources and your album.
- A refusal is not the same as being free. When the server will not place this machine on its seat, that answer is now said as what it is rather than as good news. Four different refusals all mean "stop retrying" and only one of them says the seat has ended; the others include "I cannot identify this device", which a fingerprint the server does not recognise produces on a machine whose seat is perfectly alive. Nothing local is deleted and nothing is promised on those - you are told the exit is still shut, and where the server's own words were written down.
- Send your own work without an administrator. Every generated page in the album was gated on the right to rewrite canonical metadata, so the pages Studio draws from YOUR files - the renders you added, the approved planning task you uploaded, the location scheme cut from the general plan you control - sat behind the same wall as the company's registration certificate. Who may redraw a generated page is now decided by the material it is drawn from, and the project answers who holds that material. The cover, the drawing list, a building's sub-cover, the organisation's certificate and licence and the canonical title block stay with the administrator, and a page whose material cannot be named stays there too.
- The seat dialog stopped promising something it does not do. It said the only way back out of bot state was to sign in again. Signing in restores the owner's rights and leaves the machine on its seat, which is deliberate - but the sentence sent everybody, including the person who wrote it, away from the one question worth asking. It now says what signing in actually does, and names the entry that really releases the seat.
- A machine recognises the sources it bound itself. Registering a device key changed which names this computer answers to, and one of the names it had already written into its own project files was no longer among them - so its own sources read as another machine's and were skipped. Every form of this device's identity is now accepted for that question.
- The cloud button counts all of your unsent work. Six different kinds of work wait to go up - source packages, album components, project information, company assignment, building composition and the canonical title block - and both the indicator and the summary read exactly one of them. A project with five kinds of work waiting reported that everything had been sent.
- The album list stopped hanging. Opening it could rebuild the whole album more than twenty times over and hash gigabytes of unchanged files to do it.
- Work that was skipped is no longer reported as done. A component the sync passed over was counted in the "sent" line, so a run that moved nothing could finish with a sentence saying it had.
- When the server refuses, its own words are kept. Every call now writes the refusal where it happens - the route as a symbol, the server's code exactly as it arrived including one this version has never heard of, the status, the time and how many times in a row - and clears it for that route the moment the route works again. Five explanations were built for one broken machine and every one of them fell, because the only fact that could have settled it lived for a few milliseconds and was written down nowhere.
- A project the server refuses is not a project you have lost. One refusal covered four completely different situations, and the client treated it as the end of the project. Nothing is now read as permanent unless the server says so.
- The sentence after a sync conflict says what actually happened. When your work had to be rebased onto somebody else's newer album it said so, and when it could not be it said your work would be sent anyway - which was not true and sent people back to look for it.

## [0.001.63] - 2026-09-10

- A seated machine can resume itself again. What this computer calls itself depended on who was signed in: the registered device key's fingerprint was installed by one path, and that path returns immediately when there is no owner session - which is the permanent state of a machine that holds a bot seat, because seating it erases the owner's credential on purpose. So the machine was seated under one name and reopened under another, the server looked for the first and was offered the second, and the seat could never be resumed without an owner signing in first. The person who hit it described it exactly: it switches beautifully when first created, and cannot switch back when reopened. The key's registration is a fact about the device, not about an account, and is now read as one.
- A bot seat is no longer taken off a machine because a refusal arrived. The server answers four different refusals that all mean "stop retrying"; only ONE of them says the seat has ended - the owner released it remotely - and the others include "I cannot place this device", which a fingerprint the server does not recognise also produces. Deleting the machine's seat on any of them turned one bad answer into a computer that had forgotten it was a seat while the server went on holding it, with no way back. Only the owner's remote release does that now; the rest keep the seat, drop what failed, and say so.
- Say why a resume failed, in writing. When a seated machine cannot pick its seat back up, the refusal now goes to a file beside the seat: the server's code verbatim - including one this version has never heard of - the status, the time, how many times in a row, and the first eight characters of the fingerprint that was sent. It is cleared the moment a resume succeeds. Five separate explanations were built for one person's broken machine and every one of them fell, because the only fact that could have settled it existed for a few milliseconds and was never written down anywhere - not here and not on the server.
- Stop a queued seat release from reaching a seat that was taken again since. A release that could not be sent is remembered and retried at the next sign-in. It names the seat, and a seat can be sat in twice - so a note written before somebody re-entered that same seat would, on being sent, release the one they are using now. It is dropped instead when the machine re-entered after the note was written. The trap was there all along and could not be reached: the old address carried an empty company segment that failed on the way out.
...
- Say who is acting, not what the machine is. The account line asked whether this machine holds a seat and printed the bot's name whenever it did, which stays true for as long as the machine is a seat. An owner signed in on their own seated machine was shown the bot, with their own name moved into a tooltip. The seat is a fact about the device and is still said - underneath the person's name, as what it is - while the line itself now names whoever the work is going out as.
- Remember who was here, and ask only for the password. Signing in on your own machine asked who you were every single time: the only thing that remembered was the file that signing out deletes, and that handing the machine to a seat deletes again. The address and display name are now kept for the DEVICE, in their own file, and the sign-in shows the name and a letter avatar and asks for the password alone. An e-mail is an identifier, not a credential - no password, no token and no activation is stored, and the type itself is held to that by a test rather than by today's callers. "Sign in as somebody else" goes back to the full form and keeps the profile; "stop remembering" removes it, because a seated machine can be a shared machine. Somebody already signed in does not have to sign in again for this to start working.
- Stop using one word for the job and the machine. "Бот" named both the seat an organisation pays for and the computer standing in the office, so "turn this device into a bot" read as though the machine became something. It takes a SEAT, and the seat exists without it. The menu now offers to seat the device, to return to bot state, and - said as what it costs - to release the seat and free the machine; seat management, the name field and the assignment messages all say seat where they mean seat. The rule is held by reading the menu's own labels rather than by pinning today's sentences, so improving the wording cannot turn the test red for the wrong reason.
- Stop letting a background chore answer for the person. Retrying a queued seat release wrote the status line itself, and it runs at every owner sign-in - so on a seated machine the confirmation "Эзэмшигчээр баталгаажлаа" was replaced, the instant it appeared, by a bot-seat refusal about a machine that had left bot state days earlier, which reads as the sign-in having failed. On an ordinary sign-in the collision ran the other way and the refusal was overwritten, so a seat this machine could not release was invisible on the one screen whose credential could have released it. The retry now reports and each screen composes: what the person did is said first, the chore after it, and neither is dropped.
- Keep the note on an unreleased seat current. The field recording why a queued seat release had not reached the server was filled by the first attempt and by nothing afterwards, so a sentence four days old read as evidence of what happened today - and a diagnosis made from it named the wrong cause. Every retry now records the server's error code beside its message, with a time and a count.
...
- A seated machine proves itself as the seat, not as whoever is standing at it. On a machine holding an organisation's seat somebody is often signed in beside it, and the plugins must still work under the seat's licence rather than that person's. The proof a seated device hands out is now minted with the machine's own credential, and a proof minted for a person is refused on a seat's record even when everything else about it looks right.
- Say when the other products will not be able to recognise the licence. A device that cannot write its identity - a credential store it has no rights to, most often - used to fail silently and show up as four products refusing for no visible reason. Studio now says it once, in the same words the refusing plugin will use.
- Correct a building assignment and have the shared album follow. A page's building is part of how it is filed in the cloud album, so moving a source to another building made the old filing wrong and never produced the new one: the correction took effect on the machine that made it and the shared album kept the previous arrangement indefinitely. Reassigning now re-sends the affected sources, and an album uploaded before the correction repairs itself when it is next opened.
...
- Stop a bot seeing projects it was never given. A seated machine was listing the owner's whole catalogue and would open any of it, against the rule the seat exists for: a bot takes part only in the projects assigned to it and the rest are not visible. The list is now filtered by the assignment the server holds, and opening is refused as well - hiding a row is not a boundary when a path can arrive from a recent list or a file dialog. An assignment list that has not been read hides everything rather than nothing, because "not read yet" must never behave as "no restriction".
- Prove this machine is this machine. The device fingerprint was a hash of machine and user facts - every input read by the client and sent by the client, so the server had never verified a device, only accepted its claim, and anyone could send any value and be that device. It is now the hash of a non-exportable key held in this Windows user's key store: the same 64 hex characters on the wire, but a value the machine can prove rather than assert. Registration signs a server challenge together with the account, so a captured registration cannot be replayed against somebody else, and it runs only with a live session in hand - a machine still known by an older fingerprint must be carried across by the ordinary sign-in first, or its records would be stranded under a form nothing sends any more.
- Give this device a key pair of its own. Until now a machine was identified by a fingerprint - a hash of machine and user facts, which is enough to recognise a device the server already knows and not enough to be an identity between two machines: a fingerprint can be copied and nothing about it can be signed. The device now holds a signing key and a key-agreement key, and derives a self-certifying code from the public half, so a peer can check who it is talking to without asking anybody. Nothing depends on it yet; it is the identity the peer work will inherit rather than replace.
- Give a seated machine its own credential. Entering bot state erases the owner’s, so until now a seated device could not reach the server at all - it could not say what it may open, nor report that it had locked itself. It now takes a bot-scoped, device-bound token once the PIN opens the seat, and reads back the projects that seat is assigned to. The token is never the owner’s and cannot become one.
- Delete a bot seat. A failed attempt used to leave a seat behind with no way to remove it. Deleting one releases whatever device was sitting in it, and the dialog says so before asking - and says plainly that the seat’s career history and project assignments are kept, because they hang off a bot id that is never reused.
- Show the seats that exist. Seat management read the first organisation without saying so, while the create dialog let the owner choose one - so seats made under a chosen organisation did not appear, and an empty grid gave no clue why. The organisation is now selected here too, named above the list, and "no seats in this organisation" is written out rather than left as a blank table.
- Stop a failed attempt leaving a seat behind. Creating a seat happens before the PIN is set and before the device is seated, so a failure in either later step left an orphan - and with no way to delete one, every retry added another. A seat of the same name is now reused instead of creating a second.
- Say which bot a machine is. A seated device showed nothing to distinguish it from an ordinary one once the PIN was entered, so the person at it could not tell which identity their work went out under. The account panel now reads "Бот: <name>" with the seat state beside it, the lock screen names the organisation the seat belongs to, and leaving bot state is one entry in the same menu.
- Choose a project when inviting somebody to a seat, instead of typing its id. An id is not something anyone knows by heart, and a mistyped one was refused with nothing to correct.
- Open a seated machine into its bot tile. A device that holds a seat now shows the bot and a PIN box instead of the shell, the way a shared workstation opens into a guest account, with a small second line for signing in as the owner - which asks for the full credential, because entering bot state erased it and four digits must not bring back an organisation's rights. The seat identity is sealed under the PIN rather than stored in the clear, so until the PIN is entered the machine does not own or receive as the bot at all, and a copied state file is not a working seat. Five wrong attempts lock the seat; the server is told, and the owner clears it remotely.
...
- Say when a source has deliveries waiting to be taken in. A package that arrives while the project is closed sits in its inbox until something rescans, and nothing said so - the project simply showed nothing new, which from the outside is indistinguishable from having exported the wrong thing, or from the export having failed. A source now states how many deliveries are waiting and when the newest arrived, and the project says the same once as it opens, pointing at the refresh that takes them in. The count is read from manifest headers only: whether a delivery is any good stays intake's question, asked when it is actually taken in.
- Stop the album page from closing Studio when its sheets have not arrived. Opening the album asks each page which page number it has in the built album, and building an album whose sheets are not in the library is refused - correctly, because a half-resolved album is not a document anyone should receive. But that refusal was raised where nothing caught it, so a project still waiting on a delivery could not open its album at all: the whole application closed on the click. Asking about an unbuildable album is now answered rather than thrown, the list opens with no page numbers, and building for real still refuses as before.
- Choose what size the portfolio's pages are, including keeping each drawing's own. Every page was forced to one built-in size, so a drawing put on a large sheet was shrunk to fit a smaller one - and picking that sheet in the drawing was a decision about how the drawing should be seen, which the portfolio then overruled. A masterplan drawn at 828 × 582 mm came out at 420 × 297 and stopped being readable at scale. The portfolio now has a page setup: a size of the user's choosing, or «Эх хуудасны хэмжээгээр», where each page keeps the size it was drawn on and the presentation holds pages of several sizes. The default is unchanged, and changing the setup never disturbs the pages, their order or their wording.
- Let a description written after the first export still reach its page. Seeding the caption only when a page was first imported meant the ordinary order of work - export the drawings, then write what each one shows, then export again - left the second description with nowhere to land, because the page already existed. A caption now follows the source the same way a title does: a description reaches any page nobody has captioned, including one imported before descriptions existed, and never touches a caption the user wrote or deliberately cleared.
- Start a portfolio page with the description it was authored with. A page drawn for the portfolio can carry a sentence about what it shows, and that sentence is exactly what belongs under it in a presentation - but the caption started empty and had to be typed again in Studio. It is now taken as the caption the page begins with, and only ever at that first import: the caption is printed and belongs to whoever is presenting, so changing the description at the source never rewrites one that has been written here.
