...
public static class BuildingArchitectureConceptPageLayout
{
    ...
    // The CONCEPT album cover's approval table. PDF export and the live Studio
    // preview must use these same boundaries so the cover never changes between
    // views.
    //
    // These are NOT the working-drawing cover's numbers. That one reproduces
    // what Revit drew and lives in CoverApprovalTableGrid.WorkingDrawing; the
    // two agree on every outer bound and differ by 5-8 mm on four interior
    // columns. What looked at first like a contradiction between this file's
    // Revit measurement and PFR's contract was two measurements of two
    // different covers - see CoverApprovalTableGridTests, which fails if either
    // set is ever copied onto the other.
